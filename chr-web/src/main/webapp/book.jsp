...
<body>
	...
	<div id="main_book">
		<div class="book_nav">
			<div class="book-nav-l">
				<div class="book-nav-button">
					<button class="book-nav-ls" onmousemove="findCategoryList()">
						分类&nbsp;<span class="glyphicon glyphicon-menu-down"
			...
			<div class="book-nav-r">
				<form
					action="${pageContext.request.contextPath }/bookAction_bookListByKeyWord"
					method="post" class="book-nav-rb">
					<input type="text" name="keyWord" value="${pageBean.keyWord }"
						class="search-text" placeholder="搜索书名或作者名"><input
						type="submit" name="" class="search-button" value="">
				</form>
			</div>
		</div>
		<div class="book_shop">
				...
				<c:forEach items="${pageBean.list }" var="book">
					<li><a href="#"> <img
						...
						<div class="book-shop-nb">
							<span class="shum">书名:${book.name }</span> <span class="zuozhe">作者:${book.author }</span>
							<%-- 	<span class="chuban">出版社:${book.press }</span> --%>
							<span class="chuban">分类 :${book.category.cname }</span> <span
								class="gongx">借还状态:<a href="">可借</a></span>
						</div></li>
			...
			<div class="book-pages">
				...
				<c:if test="${pageBean.currentPage!=1 }">
					<a href="javascript:void(0);"
						onclick="query(${pageBean.currentPage-1})">&lt;</a>
				</c:if>
				<!-- 显示每一页 -->
	...
	<form id="query3"
		action="${pageContext.request.contextPath}/bookAction_pageQuery"
		method="post">
		<input id="cid" type="hidden" name="cid" value=""> <input
			id="keyWord" type="hidden" name="keyWord" value=""> <input
